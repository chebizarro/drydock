...
COPY . .
RUN CGO_ENABLED=1 GOOS=linux GOARCH=amd64 go build -o /out/drydock ./cmd/drydock && \
    CGO_ENABLED=1 GOOS=linux GOARCH=amd64 go build -o /out/drydock-eval ./cmd/drydock-eval
