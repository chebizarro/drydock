...
Beads issue-tracker usage and the session-completion protocol live in the
generated `BEGIN BEADS INTEGRATION` block at the end of this file. That block
is regenerated by `bd` and is the single source of truth — do not restate it
here, or the two copies drift (they previously disagreed on `bd sync` vs
`bd dolt push` and on `--status in_progress` vs `--claim`).
