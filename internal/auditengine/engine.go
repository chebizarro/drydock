...
import (
	...
	"git.sharegap.net/cascadia/drydock/internal/agenticreview"
...
// runSCATools delegates to internal/sca, which owns the SCA toolchain execution
// and SARIF parsing (extracted so the dependency-upgrade service can reuse it
// without importing the whole audit engine). e.deps.Tools (nostrprobe.ToolRunner)
// satisfies sca.Runner structurally.
func (e *Engine) runSCATools(ctx context.Context, repoPath string, enabled bool) ([]reviewengine.Finding, error) {
	if !enabled {
		return nil, nil
	}
	return sca.Scan(ctx, e.deps.Tools, repoPath, e.logger)
}
