...
func TestHandlerOutputPrometheusFormat(t *testing.T) {
	...
	AgenticLoopTurns.Add(3)
	AgenticToolCalls.With("code.read", "ok").Inc()
	AgenticBudgetUtilization.With("turns").Observe(0.5)
	AgenticFinalizationFailures.With("budget_exceeded").Inc()
	AgenticLoopExhaustionFallbacks.Inc()
	AgenticSessionConflicts.With("version").Inc()
	AgenticSnapshotCorruption.Inc()
	AgenticStopReasons.With("review_submitted").Inc()
	...
	requireContains(t, body, "drydock_agentic_loop_turns_total 3")
	requireContains(t, body, `drydock_agentic_tool_calls_total{tool="code.read",outcome="ok"} 1`)
	requireContains(t, body, `drydock_agentic_budget_utilization_ratio_count{budget="turns"} 1`)
	requireContains(t, body, `drydock_agentic_finalization_failures_total{reason="budget_exceeded"} 1`)
	requireContains(t, body, "# TYPE drydock_agentic_loop_exhaustion_fallbacks_total counter")
	requireContains(t, body, `drydock_agentic_session_conflicts_total{type="version"} 1`)
	requireContains(t, body, "# TYPE drydock_agentic_snapshot_corruption_total counter")
	requireContains(t, body, `drydock_agentic_stop_reasons_total{reason="review_submitted"} 1`)
