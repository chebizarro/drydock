...
func parseMetaReviewOutput(raw string, findingCount int) (MetaReviewOutput, error) {
	candidate := llmutil.ExtractJSON(raw)
	...
	// Optional arrays default to empty (not nil) so a response that omits them
	// still satisfies Validate; present-but-wrong-typed fields fail decoding and
	// are re-prompted by reviewengine.CompleteStructured rather than coerced.
